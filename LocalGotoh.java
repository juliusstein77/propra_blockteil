...
public class LocalGotoh extends Gotoh_V2 {
    public void initialize() {
        ...
        for (int i = 1; i < s.length() + 1; i++) {
            for (int j = 1; j < t.length() + 1; j++) {
                ...
                mx_a[i][j] = Math.max(0, Math.max(mx_a[i - 1][j - 1] + get_score(s.charAt(i - 1), t.charAt(j - 1)), Math.max(mx_d[i][j], mx_i[i][j])));
            }
        }
        int[] max = get_max(mx_a);
    ...
    public void backtrack() {
        ...
        int i = max[0];
        int j = max[1];
        // -- start from the maximum value in the matrix and go to the first zero value or the border of the matrix
        while (mx_a[i][j] != 0) {
            // --- diagonal --- >>> match/mismatch
            if (i > 0 && j > 0 && (mx_a[i][j] == get_score(s.charAt(i - 1), t.charAt(j - 1)) + mx_a[i - 1][j - 1])) {
    ...
    public LocalGotoh(String s, String t, HashMap<String, Double> scoring_matrix, int gap_open_penalty, int gap_extend_penalty) {
